    ...
    for root, dirs, files in os.walk(pictures_folder):
        for file in files:
            if file.endswith(".jpeg") or file.endswith(".jpg") or file.endswith(".png"):
                ...
                # Modified image processing code
                img_path = os.path.join(root, file)
                img = cv2.imread(img_path)
                rgb_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                ...
                if torch.cuda.is_available():
                    rgb_tensor = torch.from_numpy(rgb_img).cuda()
                ...
                with torch.cuda.amp.autocast():
                    ...
                    if len(encodings) > 0:
                        ...
                        img_encoding_gpu = torch.tensor(img_encoding).to(device)
                        docs_encoding_gpu = torch.tensor(docs_img_encoding).to(device)
                        
                        # GPU accelerated face comparison
                        result = torch.all(torch.isclose(img_encoding_gpu, docs_encoding_gpu, rtol=0.6))
